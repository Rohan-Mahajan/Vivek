...
# ── Standalone test ───────────────────────────────────────────────────────────
# Runs ONLY when you execute `python recommender.py` directly.
# Never runs when this file is imported by app.py (Streamlit).
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("LLM Recommender — Standalone Python Test")
    print("=" * 60)

    TEST_USE_CASE = (
        "I'm building a customer support chatbot in Hindi. "
        "Needs to handle ~5,000 requests per day. "
        "Budget is under $30/month. Low latency is important."
    )
    print(f"\nTest use case:\n  {TEST_USE_CASE}\n")
    print("-" * 60)

    try:
        data = load_models("models.json")
    except FileNotFoundError:
        print("\nERROR: models.json not found.")
        print("Run this script from inside the llm-picker/ folder.")
        sys.exit(1)

    result = get_recommendation(TEST_USE_CASE, data)
    ...
    if result["success"]:
        rec = result["data"]
        bp = rec.get("best_pick", {})
        print(f"\n📋 Understood as: {rec.get('use_case_summary', 'N/A')}")
        print(f"\n✅ Best Pick : {bp.get('model_name')} ({bp.get('provider')})")
        print(f"   Reason    : {bp.get('reason')}")
        print(f"   Pricing   : {bp.get('pricing_note')}")
        print(f"   License   : {bp.get('license')}")
        print(f"   Self-host : {bp.get('self_hostable')}")
        print(f"   API string: {bp.get('api_model_string')}")
        print(f"\n   Strengths:")
        for s in bp.get("key_strengths", []):
            print(f"     · {s}")
        print(f"\n   ⚠ Watch out: {bp.get('watch_out_for')}")

        print(f"\n🔄 Alternatives:")
        for i, alt in enumerate(rec.get("alternatives", []), 1):
            print(f"  {i}. {alt['model_name']} ({alt['provider']})")
            print(f"     Pricing   : {alt.get('pricing_note')}")
            print(f"     Trade-off : {alt.get('trade_off')}")

        print(f"\n❌ Not recommended:")
        for nr in rec.get("not_recommended", []):
            print(f"  · {nr.get('model_name')}: {nr.get('reason')}")
    else:
        print(f"\n❌ FAILED: {result['error']}")

    print("\n" + "=" * 60)
    print("Test finished — this script is one-shot, exiting is normal.")
    print("For the interactive app, use:  streamlit run app.py")
    print("=" * 60)